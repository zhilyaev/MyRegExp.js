...
class MyRegExp {
    ...
    constructor(regExp, debug = false) {
        ...
        this.fill = (i, states) => {
            ...
            if (m.length > this.maxMapDeep)
                this.maxMapDeep = m.length;
            this.map.set(i, m);
        };
        this.regExp = regExp;
        this.debug = debug;
    ...
    printMarkup() {
        ...
        let s = '  ';
        let M = this.markup.split('').map(el => el === '!' ? '\x1b[36m' + el + '\x1b[0m' : el).join(s);
        console.log(M);
            ...
            for (let a of this.alphabet) {
                ...
                for (let nfaState of dfaState) {
                    ...
                    if (this.nfaFinals.includes(+nfaState)) {
                        if (this.debug)
                            console.log(`\t\t\t\t ${nfaState} это финальное состояние у него нет переходов`);
    ...
    // Тестирование DFA
    static testDFA(input, dfa, dfaFinals) {
        let s = 0;
    ...
    // Тестирование NFA
    static testNFA(input, nfa, nfaFinals) {
        // TODO
    }
}
